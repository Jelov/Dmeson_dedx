...
{//{{{
    ...
    private:
        ...
        //        edm::EDGetTokenT< edm::ValueMap<reco::DeDxData> > m_dEdxDiscrimTag;
...
{//{{{
    ...
    m_tracksTag     = consumes< reco::TrackCollection >(iConfig.getParameter<edm::InputTag>("RecoTrackLabel"));
    //    m_dEdxDiscrimTag= consumes< edm::ValueMap<reco::DeDxData> >(iConfig.getParameter<edm::InputTag>("RecoDEdxLabel"));
...
void Dfinder::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    //    iEvent.getByToken(m_dEdxDiscrimTag, dEdxTrackHandle);
    const edm::ValueMap<reco::DeDxData> dEdxTrack = *dEdxTrackHandle.product();
    */
    //CLEAN all memory
    memset(&PidInfo     ,0x00,sizeof(PidInfo)   );
        ...
        if (0){
            ...
            if (input_tracks.size() == 0){
                ...
                for(std::vector<pat::GenericParticle>::const_iterator tk_it=input_tracks.begin();
                    ...
                    reco::TrackRef track = tk_it->track();
                    ...
                    PidInfo.index          [PidInfo.size] = PidInfo.size;
                    //                   PidInfo.handle_index   [PidInfo.size] = index_track;
                    PidInfo.charge         [PidInfo.size] = track->charge();
                    PidInfo.p              [PidInfo.size] = track->p();
                    PidInfo.pt             [PidInfo.size] = track->pt();
                    PidInfo.eta            [PidInfo.size] = track->eta();
                    PidInfo.dedx           [PidInfo.size] = dEdxTrack[track].dEdx();
...
/*
                ...
                for(unsigned int index_track=0; index_track<trackCollection.size(); index_track++){
//                    reco::TrackRef track  = reco::TrackRef( trackCollectionHandle, index_track );
                    reco::TrackRef track = pat::genericParticle->track();
                    std::vector<int> listOfRelativeDCand1Pid;//1~nXb
                    for(int d=0; d<DInfo.size; d++){
                        int index_track_s = (int)index_track;
                        if(DInfo.rftk1_index[d] == index_track_s){
                            listOfRelativeDCand1Pid.push_back(d+1);
                        }// end if DInfo.rftk1_index[d] == index_track
                    }// end for d<DInfo.size
                    if(dropUnusedTracks_ && listOfRelativeDCand1Pid.size() == 0) continue; 
                    // TrackInfo.index          [TrackInfo.size] = TrackInfo.size;
                    // TrackInfo.handle_index   [TrackInfo.size] = tk_hindex;
                    // TrackInfo.charge         [TrackInfo.size] = tk_it->charge();
                    // TrackInfo.pt [TrackInfo.size] = tk_it->pt();
                    ...
                    for(unsigned int iCands=0; iCands < listOfRelativeDCand1Pid.size(); iCands++){
                        DInfo.rftk1_index[listOfRelativeDCand1Pid[iCands]-1] = PidInfo.size;
                    } // end for iCands < listOfRelativeDCand1Pid.size()
                    ...
                    //dEdxTrack[track].dEdx();
                    //dEdxTrack[track].numberOfSaturatedMeasurements();
                    //dEdxTrack[track].numberOfMeasurements();


                }// end for int index_track <trackCollection.size()
*/
